...
class Transfer extends BaseModel implements Document
{
    ...
    protected $appends = ['profit', 'sender_party_name', 'receiver_party_name', 'office_name', 'user_name'];
    ...
    public function confirm()
    {
        ...
        if ($this->type == 1) {
            ...
            if ($this->delivering_type == 2 || $this->delivering_type == 5) {
    ...
    public function logMoneyGramAmount($entry)
    {
        ...
        // transfer amount
        if ($this->delivering_type == 5) {
            $ben_party = Party::find($this->moneygram_on_beneficiary_id);
            $transactions[] = [
                'account_id' => $ben_party->account_id,  // on beneficiary
        ...
        if ($this->transfer_commission > 0) {
            ...
            if ($this->delivering_type == 5) {
                $transactions[] = [
                    'account_id' =>  $ben_party->account_id,
                    'amount' => $t_amount,
            ...
            $transactions[] = [
                ...
                'amount' => $t_amount,
                'ac_amount' => $transfer_commission_inDoller,
                'transaction_type' => $this->delivering_type == 5 ? 18 : 16,
                'exchange_rate' => $this->transfer_commission_exchange_rate,
                'currency_id' => $this->transfer_commission_currency,
                'type' =>  0,
            ];
    ...
    public  function logAmount($entry)
    {
        ...
        $account_id =  $this->user_account_id; // $entry->ref_currency->account_id;
        $doller_account_id =  $this->get_user_account_id(1); // $entry->ref_currency->account_id;
        $sender_account_id = $this->sender_party->account_id; // $entry->ref_currency->account_id;
        $party_office_box = $this->get_user_account_id($this->office_currency_id);
        // transfer amount
        if ($this->delivering_type != 3) {
            $transactions[] = [
                ...
                'amount' => $sender_amount,
                'ac_amount' => $sender_amount * $this->exchange_rate_to_delivery_currency,
                'transaction_type' =>  2, // 6  include to net_balance
                'exchange_rate' => $this->exchange_rate_to_delivery_currency,
                'currency_id' => $this->delivery_currency_id,
                'type' => 0,
            ];
            ...
            $transactions[] = [
                'account_id' => $sender_account_id, // $this->user_account_id
                'amount' => $sender_amount,
                'ac_amount' => $sender_amount * $this->exchange_rate_to_delivery_currency,
                'transaction_type' => 3, // 6  include to net_balance
                'exchange_rate' => $this->exchange_rate_to_delivery_currency,
                'currency_id' => $this->delivery_currency_id,
                'type' => 0,
            ];
        ...
        if ($this->transfer_commission != 0) {
            ...
            if ($this->delivering_type != 3) {
                $transactions[] = [
                    'account_id' => $transfer_commission_box_id, //$transfer_profit_account_id,
                    'amount' => $t_amount,
                    'ac_amount' => $transfer_commission_inDoller,
                    'transaction_type' => 4,
                    'exchange_rate' => $this->transfer_commission_exchange_rate,
                ...
                $transactions[] = [
                    'account_id' => $sender_account_id, //$transfer_profit_account_id,
                    'amount' => $t_amount,
                    'ac_amount' => $transfer_commission_inDoller,
                    'transaction_type' => 5,
                    'exchange_rate' => $this->transfer_commission_exchange_rate,
        ...
        $office_commission_box_id = $this->get_user_account_id($this->office_currency_id);
        ...
        if ($this->office_commission > 0) {
            ...
            $transactions[] = [
                ...
                'amount' => $o_amount,
                'transaction_type' =>  $this->delivering_type != 3 ? 9 : 10,
            ...
            $transactions[] = [
                'account_id' => $office_commission_box_id,
                'ac_amount' =>  $office_commission_amount_usd,
                'amount' => $o_amount,
                'transaction_type' =>  $this->delivering_type != 3 ? 9 : 10,
                'on_account_balance_id' => $this->get_user_account_id($this->office_currency_id),
        ...
        if ($this->returned_commission > 0) {
            ...
            $transactions[] = [
                ...
                'account_id' => $office_commission_box_id, //$commission_box_id,
                'amount' =>  $f_amount, //$f_amount,
                'ac_amount' => $returned_commission_amount_usd,
                'transaction_type' => $this->delivering_type != 3 ? 7 : 8,
                'on_account_balance_id' => $this->get_user_account_id($this->office_currency_id),
                'currency_id' =>  $this->office_currency_id,
                'exchange_rate' =>  $office_exchange_rate,
                'type' =>  0,
            ...
            $transactions[] = [
                // 'account_id' => $transfer_commission_box_id, //$commission_box_id,
                'account_id' => $this->office->account_id, //$commission_box_id,
                'amount' =>  $f_amount, //$f_amount,
                'ac_amount' => $returned_commission_amount_usd,
                'transaction_type' => $this->delivering_type != 3 ? 7 : 8,
                'on_account_balance_id' => $this->get_user_account_id($this->office_currency_id),
        ...
        $party_amount = $this->office_amount_in_office_currency + $this->returned_commission - $this->office_commission;
        $transactions[] = [
            'account_id' => $party_office_box,
            'amount' => $party_amount,
            'ac_amount' =>   $party_amount,
            'transaction_type' => 6,
            'currency_id' => $this->office_currency_id,
    ...
    public  function logIncomingAmount($entry)
    {
        ...
        $user_box_id = $this->get_user_account_id($this->on_dollar_account ? 1 : $this->office_currency_id);
        ...
        $transactions[] = [
            ...
            'amount' => $this->on_dollar_account ? $this->office_amount : $amount,
            'ac_amount' => $this->office_amount,
            'transaction_type' => 11,
        ...
        // giving amount 
        $transactions[] = [
            'account_id' => $this->get_user_account_id($this->received_currency_id),
            ...
            'transaction_type' => $this->delivering_type == 4 ?  13 : 12,
            'type' => 1,
        ];
    ...
    public  function logIncomingMoneyGramAmount($entry)
    {
        ...
        $transactions[] = [
            'account_id' => $this->get_user_account_id(1), // $this->user_account_id moew moew moew moew moew
            'amount' => $this->on_dollar_account ? ($amount * $this->exchange_rate_to_office_currency) : $amount,
            'ac_amount' => $amount * $this->exchange_rate_to_office_currency,
            'transaction_type' => 21,
            'currency_id' => 1,
